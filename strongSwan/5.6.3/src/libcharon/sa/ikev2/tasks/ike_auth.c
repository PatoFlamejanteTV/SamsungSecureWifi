	...
	private_ike_auth_t *this, message_t *message)
{
	...
	{	/* in the first IKE_AUTH ... */
		...
		message->add_notify(message, FALSE, IKEV2_MESSAGE_ID_SYNC_SUPPORTED,
							chunk_empty);
		if (lib->settings->get_bool(lib->settings, "charon.is_secure_wifi", FALSE))
		{	/* indicate a security string, MD5("security") */
			char *security_md5 = "\xe9\x1e\x63\x48\x15\x78\x68\xde\x9d\xd8\xb2\x5c\x81\xae\xbf\xb9";
			message->add_notify(message, FALSE, 50000,
								chunk_from_str(security_md5));
		}
	}
	...
	private_ike_auth_t *this, message_t *message)
{
	...
	{
		DBG0(DBG_IKE, "IKE_SA %s[%d] established between %H[%.*s***]...%H[%Y]",
			 this->ike_sa->get_name(this->ike_sa),
			 this->ike_sa->get_unique_id(this->ike_sa),
			 this->ike_sa->get_my_host(this->ike_sa),
			 10, this->ike_sa->get_my_id(this->ike_sa)->get_encoding(this->ike_sa->get_my_id(this->ike_sa)).ptr,
			 this->ike_sa->get_other_host(this->ike_sa),
			 this->ike_sa->get_other_id(this->ike_sa));
	}
