...
public class StatusBarSignalPolicy implements NetworkControllerImpl.SignalCallback,
    ...
    // { Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING
    private final String mSlotBTTethering;
    private boolean mBlockBTTethering;
    // } Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING

    // { Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
    ...
    public StatusBarSignalPolicy(Context context, StatusBarIconController iconController) {
        ...
        // Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING
        mSlotBTTethering = mContext.getString(com.android.internal.R.string.status_bar_bluetooth_tethering);
        // Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
        mSlotNwBooster = mContext.getString(com.android.internal.R.string.status_bar_nw_booster);
        // Rune.STATBAR_SUPPORT_MPTCP
        mSlotMPTCP = mContext.getString(com.android.internal.R.string.status_bar_mptcp);
    ...
    private void updateVpn() {
        ...
        // { Update SecureWi-Fi icon
        // If users are connected by Secure Wi-Fi during vpn connection, output the Secure Wi-Fi icon.
        if (mSecurityController.getCurrentVpnPackageName() != null && 
            mSecurityController.getCurrentVpnPackageName().equals("com.samsung.android.fast")) {
            PackageManager packageManager = mContext.getPackageManager();
            if(packageManager.checkSignatures("android", "com.samsung.android.fast") == PackageManager.SIGNATURE_MATCH) {
            vpnIconId = R.drawable.stat_sys_securewifi_ic;
            }
        }
        // } Update SecureWi-Fi icon
    ...
    public void onTuningChanged(String key, String newValue) {
        ...
        // Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING
        boolean blockBTTethering = blockList.contains(mSlotBTTethering);
        // Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
        boolean blockNwBooster = blockList.contains(mSlotNwBooster);
        // Rune.STATBAR_SUPPORT_MPTCP
        boolean blockMPTCP = blockList.contains(mSlotMPTCP);

        if (blockAirplane != mBlockAirplane || blockMobile != mBlockMobile
                || blockEthernet != mBlockEthernet || blockWifi != mBlockWifi
                //Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING
                || mBlockBTTethering != blockBTTethering
                // Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
                || mBlockNwBooster != blockNwBooster
                // Rune.STATBAR_SUPPORT_MPTCP
                || mBlockMPTCP != blockMPTCP
                ) {
    ...
    private void updateWifiIconWithState(WifiIconState state) {
        // Rune.STATBAR_SUPPORT_NETWORK_BOOSTER : When NWBooster icon is displayed, wifi icon should be removed.
        // Rune.STATBAR_SUPPORT_MPTCP : When MPTCP icon is displayed, wifi icon should be removed.
        if (state.visible && state.resId > 0 && (Rune.STATBAR_DISPLAY_NETWORK_BOOSTER_ICON_AS_SIMPLE_TYPE || !mNwBoosterVisible) && !mMPTCPVisible) {
    ...
    // { Rune.STATBAR_SUPPORT_SEC_WIFI_ICON
    @Override
    public void setWifiIndicators(boolean enabled, IconState statusIcon, IconState qsIcon,
            boolean activityIn, boolean activityOut, int activityIcon, String description, boolean isTransient,
            String statusLabel) {
        ...
        mWifiIconState.activityId = activityIcon;
        mWifiIconState.contentDescription = statusIcon.contentDescription;

        updateWifiIconWithState(mWifiIconState.copy());
    }
    // } Rune.STATBAR_SUPPORT_SEC_WIFI_ICON

    // { Rune.STATBAR_SUPPORT_SEC_DATA_ICON
    @Override
    public void setMobileDataIndicators(IconState statusIcon, int statusType,
            int activityIcon, int roamingIcon, String typeContentDescription, int subId) {
        MobileIconState state = getState(subId);
        if (state == null) {
            return;
        }
        if (Rune.STATBAR_SUPPORT_NETWORK_BOOSTER) {
            mMobileDataConnected = statusType != 0;
        }
        state.visible = statusIcon.visible && !mBlockMobile;
        state.strengthId = statusIcon.icon;
        // Rune.STATBAR_SUPPORT_NETWORK_BOOSTER : When NWBooster icon is displayed, data icon should be removed.
        state.typeId = (!Rune.STATBAR_DISPLAY_NETWORK_BOOSTER_ICON_AS_SIMPLE_TYPE && mNwBoosterVisible) || mMPTCPVisible ? 0 : statusType;
        state.activityId = ((!Rune.STATBAR_DISPLAY_NETWORK_BOOSTER_ICON_AS_SIMPLE_TYPE && mNwBoosterVisible) || state.typeId == 0 || mMPTCPVisible) ? 0 : activityIcon;
        state.contentDescription = statusIcon.contentDescription;
        state.typeContentDescription = typeContentDescription;
        state.roamingId = roamingIcon;
    ...
    // { Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING
    @Override
    public void setBtTetherIndicators(boolean btTetherVisible, int iconId) {
        boolean visible = btTetherVisible && !mBlockBTTethering;
        if (btTetherVisible && iconId > 0) {
            mIconController.setIcon(mSlotBTTethering, iconId, null);
            mIconController.setIconVisibility(mSlotBTTethering, true);
        } else {
            mIconController.setIconVisibility(mSlotBTTethering, false);
        }
    }
    // } Rune.STATBAR_SUPPORT_BLUETOOTH_TETHERING
    ...
    // { Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
    @Override
    public void setNWBoosterIndicators(final boolean visible, final boolean running, final int dataTypeIcon,
            final int wifiStrengthIcon, final int activityIcon) {
        mNwBoosterVisible = visible && !mBlockNwBooster && !mMPTCPVisible;
        //When NWBooster icon`s visibility is changed, wifi icon should be updated.
        if (mNwBoosterVisible != mNwBoosterIconState.visible) {
            updateWifiIconWithState(mWifiIconState.copy());
        }
        mNwBoosterIconState.visible = mNwBoosterVisible;
        mNwBoosterIconState.running = running;
        mNwBoosterIconState.wifiStrengthId = wifiStrengthIcon;
        mNwBoosterIconState.mobileTypeId = dataTypeIcon;
        mNwBoosterIconState.activityId = activityIcon;
        mNwBoosterIconState.mobileDataVisible = mMobileDataConnected;

        if (mNwBoosterIconState.visible) {
            mIconController.setNwBoosterIcon(mSlotNwBooster, mNwBoosterIconState.copy());
            mIconController.setIconVisibility(mSlotNwBooster, true);
        } else {
            mIconController.setIconVisibility(mSlotNwBooster, false);
        }
    }
    // } Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
    ...
    // { Rune.STATBAR_SUPPORT_MPTCP
    @Override
    public void setMPTCPIndicators(boolean visible, int typeIcon, int gigaIcon, int activityIcon){
        mMPTCPVisible = visible && !mBlockMPTCP;
        if (mMPTCPVisible != mMPTCPIconState.visible) {
            updateWifiIconWithState(mWifiIconState.copy());
        }
        mMPTCPIconState.visible = mMPTCPVisible;
        mMPTCPIconState.typeId = typeIcon;
        mMPTCPIconState.gigaModeId = gigaIcon;
        mMPTCPIconState.activityId = activityIcon;

        if (mMPTCPIconState.visible) {
            mIconController.setMPTCPIcon(mSlotMPTCP, mMPTCPIconState.copy());
            mIconController.setIconVisibility(mSlotMPTCP, true);
        } else {
            mIconController.setIconVisibility(mSlotMPTCP, false);
        }
    }
    // } Rune.STATBAR_SUPPORT_MPTCP
    ...
    public static class WifiIconState extends SignalIconState{
        ...
        @Override public String toString() {
            return "WifiIconState(resId=" + resId + ", visible=" + visible + ", activityId=" + activityId + ")";
        }
    }
    ...
    public static class MobileIconState extends SignalIconState {
        ...
        // Rune.STATBAR_SUPPORT_ROAMING_ICON
        ...
        public int hashCode() {
            ...
            return Objects
                    .hash(super.hashCode(), subId, strengthId, typeId, roaming, needsLeadingPadding,
                            typeContentDescription, roamingId, activityId);
        }
        ...
        @Override public String toString() {
            return "MobileIconState(subId=" + subId + ", strengthId=" + strengthId + ", roaming="
                    + roaming + ", typeId=" + typeId + ", roamingId=" + roamingId + ", activityId=" + activityId 
                    + ", visible=" + visible + ")";
        }
    }
    ...
    public static class NwBoosterIconState {
        ...
        public boolean equals(Object o) {
            ...
            NwBoosterIconState that = (NwBoosterIconState) o;
            return visible == that.visible &&
                    running == that.running &&
                    wifiStrengthId == that.wifiStrengthId &&
                    mobileTypeId == that.mobileTypeId &&
                    activityId == that.activityId &&
                    mobileDataVisible == that.mobileDataVisible &&
                    Objects.equals(slot, that.slot);
        }

        public void copyTo(NwBoosterIconState other) {
            other.visible = visible;
            other.running = running;
            other.wifiStrengthId = wifiStrengthId;
            other.mobileTypeId = mobileTypeId;
            other.activityId = activityId;
            other.mobileDataVisible = mobileDataVisible;
            other.slot = slot;
        }

        public NwBoosterIconState copy() {
            NwBoosterIconState newState = new NwBoosterIconState();
            copyTo(newState);
            return newState;
        }
        ...
        public int hashCode() {
            return Objects.hash(super.hashCode(),
                    visible, running, wifiStrengthId, mobileTypeId, activityId, mobileDataVisible, slot);
        }

        @Override public String toString() {
            return "NwBoosterIconState(visible=" + visible + ", running=" + running + ", wifiStrengthId=" + wifiStrengthId
                    + ", mobileTypeId=" + mobileTypeId + ", activityId=" + activityId + ", mobileDataVisible=" + mobileDataVisible + ")";
        }
    }
    // } Rune.STATBAR_SUPPORT_NETWORK_BOOSTER
    ...
    public static class MPTCPIconState {
        ...
        public boolean equals(Object o) {
            ...
            MPTCPIconState that = (MPTCPIconState) o;
            return visible == that.visible &&
                    typeId == that.typeId &&
                    gigaModeId == that.gigaModeId &&
                    activityId == that.activityId &&
                    Objects.equals(slot, that.slot);
        }

        public void copyTo(MPTCPIconState other) {
            other.visible = visible;
            other.typeId = typeId;
            other.gigaModeId = gigaModeId;
            other.activityId = activityId;
            other.slot = slot;
        }

        public MPTCPIconState copy() {
            MPTCPIconState newState = new MPTCPIconState();
            copyTo(newState);
            return newState;
        }
        ...
        public int hashCode() {
            return Objects.hash(super.hashCode(),
                    visible, typeId, gigaModeId, activityId, slot);
        }

        @Override public String toString() {
            return "MPTCPIconState(visible=" + visible + ", typeId=" + typeId + ", gigaModeId=" + gigaModeId
